...
export default async function DashboardPage() {
  ...
  const [user, activities, monthlyVae, monthlySc, recentVae, monthlyScActivities] = await Promise.all([
  ...
  return (
    <main className="dashboard-shell">
      <header className="dashboard-header">
        ...
        <div className="dashboard-avatar" aria-label={`Usuario ${user.firstName} ${user.lastName}`}>
      ...
      <section className="dashboard-section" aria-labelledby="upcoming-title">
        <div className="dashboard-section-heading">
          ...
          <div className="dashboard-actions">
            <Link className="dashboard-action dashboard-action-house" href="/dashboard/households" aria-label="Crear hogar" title="Crear hogar">
              <House size={17} strokeWidth={2} aria-hidden="true" />
            </Link>
            <Link className="dashboard-action dashboard-action-house" href="/dashboard/households/due" aria-label="Ver Santa Cenas" title="Santa Cenas">
              <CalendarClock size={17} strokeWidth={2} aria-hidden="true" />
            </Link>
            <Link className="dashboard-action dashboard-action-house" href="/dashboard/reports" aria-label="Exportar actividades a Excel" title="Exportar actividades a Excel">
              <FileSpreadsheet size={17} strokeWidth={2} aria-hidden="true" />
            </Link>
            <Link className="dashboard-action" href="/dashboard/activities/new">+ Nueva actividad</Link>
          </div>
        </div>
                  ...
                  <div className="activity-actions">
                    ...
                    {activity.activityType.code === "SC" && (
                      <form action={completeActivity}>
                        ...
                        <button type="submit">Marcar visitado</button>
                      </form>
                    )}
